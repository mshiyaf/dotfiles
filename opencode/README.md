...
- **`crew`** - a lightweight tmux/herdr orchestrator (our own take on firstmate, no external scripts):
  `crew new "<task…>"` spins up a worktree + detached tmux session or herdr workspace running an agent, with the
